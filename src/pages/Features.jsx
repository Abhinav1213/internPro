  ...
  useEffect(() => {
    const fetchStocks = async () => {
      try {
        ...
        const cachedTimestamp = localStorage.getItem("stocksDataTimestamp");

        const now = new Date().getTime();
        const twelveHours = 12 * 60 * 60 * 1000; // 12 hours in milliseconds

        // If cached data exists and is not expired, use it
        if (cachedData && cachedTimestamp && now - cachedTimestamp < twelveHours) {
        ...
        const response = await fetch("https://api.skyliferesearch.com/stocks-list", {
          method: "GET",
          headers: {
        ...
        localStorage.setItem("stocksData", JSON.stringify(shownData));
        localStorage.setItem("stocksDataTimestamp", now.toString());
